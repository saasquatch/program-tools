...
var ReferralComponent = /** @class */ (function () {
    ...
    ReferralComponent.prototype.getName = function () {
        var referral = this.referral;
        var referredByReferral = this.referral;
        // Referred User
    ...
    ReferralComponent.prototype.getIcon = function () {
    ...
    ReferralComponent.prototype.getContent = function (formatVariables) {
    ...
    ReferralComponent.prototype.rewardIsExpired = function () {
        var rewards = this.referral.rewards;
        var hasExpiry = rewards.length == 1 && rewards[0].statuses;
        var isExpired = hasExpiry && rewards[0].statuses.indexOf("EXPIRED") > -1;
        return isExpired;
    };
    ReferralComponent.prototype.rewardIsCancelled = function () {
        var rewards = this.referral.rewards;
        var hasStatuses = rewards.length == 1 && rewards[0].statuses;
        var isCancelled = hasStatuses && rewards[0].statuses.indexOf("CANCELLED") > -1;
        return isCancelled;
    };
    ReferralComponent.prototype.getValue = function () {
        var rewards = this.referral.rewards;
        var referrer = this.referralvariables.referrervalue;
        var pending = this.referralvariables.pendingvalue;
    ...
    ReferralComponent.prototype.getValueContent = function (formatVariables) {
        var rewards = this.referral.rewards;
    ...
    ReferralComponent.prototype.render = function () {
        var _a = this.referral, dateReferralStarted = _a.dateReferralStarted, rewards = _a.rewards;
        var formatVariables = {
        ...
        var content = this.getContent(formatVariables);
        var value = this.getValue();
        var valuecontent = this.getValueContent(formatVariables);
        return (h("div", { class: "squatch-referrals-row" }, h("div", null, h("div", { class: "squatch-referrals-heading" }, name), h("div", { class: "squatch-referrals-description" }, content)), h("i", { class: "icon squatch-referrals-icon " + icon + " " + (this.rewardIsExpired() && 'expired') + " " + (this.rewardIsCancelled() && 'cancelled') }), h("div", { class: "sqh-column-two" }, h("div", { class: "squatch-referrals-value " + (this.rewardIsCancelled() && 'cancelled') + " " + (rewards.length > 0 ? this.rewardIsExpired() ? 'expired' : '' : this.referraltype === 'referrer' ? 'referrer' : 'pending') }, value), h("div", { class: "squatch-value-contents " + (this.rewardIsExpired() && 'expired') + " " + (this.rewardIsCancelled() && 'cancelled') }, valuecontent))));
    };
    Object.defineProperty(ReferralComponent, "is", {
        get: function () { return "sqh-referral-component"; },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ReferralComponent, "properties", {
        get: function () {
            return {
                "referral": {
                    "type": "Any",
...
var ReferralList = /** @class */ (function () {
    ...
    ReferralList.prototype.componentWillLoad = function () {
        var _this = this;
        if (!this.ishidden) {
            return this.getReferrals().then(function (res) {
                _this.referrals = res.referrals.data;
                _this.referredBy = res.referredByReferral;
                _this.referralsCount = res.referrals.totalCount;
                _this.loading = false;
            }).catch(function (e) {
                _this.onError(e);
            });
    ...
    ReferralList.prototype.getReferrals = function (offset) {
        if (offset === void 0) { offset = 0; }
        return API.graphql.getReferrals(offset);
    };
    ReferralList.prototype.paginate = function (offset, event) {
        var _this = this;
        if (this.loading)
            return null;
        var referralsCount = this.referralsCount;
        ...
        this.getReferrals(offset)
            .then(function (res) {
            target.innerText = offset > _this.offset
                ? _this.paginatemore
                : _this.paginateless;
            _this.referrals = res.referrals.data;
            _this.offset = offset;
    ...
    ReferralList.prototype.onError = function (e) {
        console.error(e);
        this.loading = false;
    };
    ReferralList.prototype.render = function () {
        var _this = this;
        var referredByRow;
        var referralsRow;
        var referralvariables = {
        ...
        if (this.referrals) {
            referralsRow = (this.referrals.map(function (ref) {
                var referraltype = ref.rewards.length > 0 ? 'converted' : 'pending';
                return (h("sqh-referral-component", { id: uuid(), referral: ref, referralvariables: referralvariables, referraltype: referraltype, unknownuser: _this.unknownuser }));
            }));
        }
            ...
            h("div", { class: "squatch-referrals " + clz }, h("div", { class: "squatch-referrals-scroll-container" }, referralsRow, referredByRow), h("div", { class: "squatch-referrals-scroll-action-container" }, h("button", { class: "\n                squatch-referrals-scroll-action previous " + (this.loading
                    ...
                    : this.showreferrer && this.referredBy
                        ...
                        : this.offset >= this.referralsCount - 3
                            ? "disabled"
                            : "") + "\n              ", "view-more": true, onClick: function (event) { return _this.paginate(_this.offset + 3, event); } }, this.paginatemore))))
            // 'No Referrals Yet' button if this.referralCount < 1
            : (h("div", { class: "squatch-referrals-scroll-action-container" }, h("button", { disabled: true, class: 'squatch-no-referrals-yet' }, this.noreferralsyet)));
    };
    Object.defineProperty(ReferralList, "is", {
        get: function () { return "sqh-referral-list"; },
        enumerable: true,
        configurable: true
    });
    Object.defineProperty(ReferralList, "properties", {
        get: function () {
            return {
                "cancelledcolor": {
                    "type": String,
    ...
    Object.defineProperty(ReferralList, "style", {
