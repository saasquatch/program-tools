...
const componentBoilerplateMock = vi.hoisted(() => ({
  ...
  useQuery: vi.fn(() => ({ data: { user: { shareLink: 'https://example.com/share' } }, loading: false })),
...
describe('useShareLink', () => {
  beforeEach(() => {
    componentBoilerplateMock.useQuery.mockReturnValue({
      data: { user: { shareLink: 'https://example.com/share' } },
      loading: false,
    });
    componentBoilerplateMock.useMutation.mockReturnValue([sendLoadEventMock]);
  });

  it('returns the share link as copyString', () => {
    const result = useShareLink({ tooltipLifespan: 1000 } as any);

    expect(result.copyString).toBe('https://example.com/share');
  });
