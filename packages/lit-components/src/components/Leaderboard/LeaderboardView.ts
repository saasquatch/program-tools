...
export function LeaderboardView(props: LeaderboardProps & LeaderboardHookResult) {
  return html`
    <style>
      ...
      .leaderboard-header { font-weight: var(--sl-font-weight-semibold); padding: var(--sl-spacing-small); border-bottom: 2px solid var(--sl-color-neutral-200); text-align: left; }
      .leaderboard-row { border-bottom: 1px solid var(--sl-color-neutral-100); }
      .leaderboard-row:hover { background: var(--sl-color-neutral-50); }
      .leaderboard-cell { padding: var(--sl-spacing-small) var(--sl-spacing-medium); }
      .rank-cell { width: 50px; text-align: center; font-weight: var(--sl-font-weight-semibold); }
      .name-cell { width: 100%; }
      .value-cell { text-align: right; font-weight: var(--sl-font-weight-semibold); }
              ...
              <tbody>
                ${props.rows.map(
                  (row, index) => html`<tr class="leaderboard-row">
                    ${props.showRank ? html`<td class="leaderboard-cell rank-cell">${row.rank || index + 1}</td>` : null}
                    ${props.showUser ? html`<td class="leaderboard-cell name-cell">${formatName(row)}</td>` : null}
                    ${props.showStats ? html`<td class="leaderboard-cell value-cell">${formatValue(row)}</td>` : null}
                  </tr>`
                )}
              </tbody>
            </table>`}
