...
@Component({
  tag: 'sqp-rewards-table-source-cell',
  shadow: true,
})
export class RewardTableSourceCell {
  ...
  render() {
    ...
    const RewardExchangeBadge = ({ reward }: { reward: Reward }) => {
      ...
      return reward.exchangedRewardRedemptionTransaction.prettyRedeemedCredit.length + reward.prettyValue.length < SOURCE_COLUMN_LENGTH ? (
        rewardExchange
      ) : (
        <div
          style={{
    ...
    const Source = () =>
      this.reward.rewardSource === 'FRIEND_SIGNUP' || this.reward.rewardSource === 'REFERRED' ? (
        <div>
          <div
            style={{
              fontSize: 'var(--sl-font-size-small)',
