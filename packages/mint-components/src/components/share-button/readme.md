# sqm-share-button



<!-- Auto Generated Below -->


## Properties

| Property     | Attribute    | Description                                                                                                               | Type                                                                               | Default     |
| ------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------- |
| `disabled`   | `disabled`   |                                                                                                                           | `boolean`                                                                          | `undefined` |
| `hideicon`   | `hideicon`   |                                                                                                                           | `boolean`                                                                          | `false`     |
| `iconslot`   | `iconslot`   |                                                                                                                           | `"prefix" \| "suffix"`                                                             | `'prefix'`  |
| `medium`     | `medium`     | The social medium to share on. Share messages and links will be pulled from your program config and tagged for analytics. | `"TODO" \| "email" \| "facebook" \| "twitter"`                                     | `undefined` |
| `pill`       | `pill`       |                                                                                                                           | `boolean`                                                                          | `undefined` |
| `programId`  | `program-id` | Optional programId, or uses the programId context where this button is rendered.                                          | `string`                                                                           | `undefined` |
| `sharetext`  | `sharetext`  |                                                                                                                           | `string`                                                                           | `undefined` |
| `sharetitle` | `sharetitle` |                                                                                                                           | `string`                                                                           | `undefined` |
| `size`       | `size`       |                                                                                                                           | `"large" \| "medium" \| "small"`                                                   | `undefined` |
| `type`       | `type`       |                                                                                                                           | `"danger" \| "default" \| "info" \| "primary" \| "success" \| "text" \| "warning"` | `'primary'` |


----------------------------------------------

*Built with [StencilJS](https://stenciljs.com/)*
