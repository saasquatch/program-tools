...
| `statusText`       | `status-text`       |                                                                  | `string` | `"{status, select, AVAILABLE {Available} CANCELLED {Cancelled} PENDING {Pending} EXPIRED {Expired} REDEEMED {Redeemed} PENDING_REVIEW {Pending Review} DENIED {Denied} other {Not available} }"` |
