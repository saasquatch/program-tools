...
async function render(pathname) {
    ...
    if (lastRenderedLayoutPath.length > 0) {
        lastRenderedLayoutPath[lastRenderedLayoutPath.length - 1].element.innerHTML = "";
    }
    // Apply the layouts in reverse
    let layouts = [...page.layouts].reverse();
    layouts.forEach((layout, i) => {
        var _a;
        if (((_a = lastRenderedLayoutPath[i]) === null || _a === void 0 ? void 0 : _a.key) === layout.key) {
...
function main() {
    function wrapHistoryFunction(functionName) {
    ...
    window.addEventListener("replaceState", () => render(window.location.pathname));
